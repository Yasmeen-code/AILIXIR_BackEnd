...
class AwardCompleteSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($awards as $awardData) {
            // 1. إنشاء الـ Award بدون صور أولاً
            $award = Award::create([
                'name' => $awardData['name'],
                ...
                'website' => $awardData['website'],
                'images' => [] // نبدأ بـ array فاضية
            ]);

            // 2. جمع الصور المحلية (نفس منطق AwardSeeder القديم)
            ...
            // 3. رفع الصور على Cloudinary وتحديث الـ Award
            $cloudinaryUrls = [];
            ...
            // 4. حفظ الـ URLs في الـ Award
            if (!empty($cloudinaryUrls)) {
                $award->images = $cloudinaryUrls;
                $award->save();
                $this->command->info("✓ Created: {$award->name} with " . count($cloudinaryUrls) . " images");
            } else {
